...
//extern bool ov12a10b_read_otp_pdaf_data( kal_uint16 addr, BYTE* data, kal_uint32 size);
/*
        ...
        do {
            *sensor_id = return_sensor_id();
			...
			if(*sensor_id == 0x1241)
			{
				if(flag_cam == 11){
					*sensor_id = 0x124b;
					printk("ov12a10B yazhou_debug flag_cam = %d , *sensor_id = %x\n",flag_cam,*sensor_id);
				}else{
					*sensor_id = 0x1241;
					printk("ov12a10B yazhou_debug flag_cam = %d , *sensor_id = %x\n",flag_cam,*sensor_id);
				}
...
static kal_uint32 open(void) //check out
{
    ...
    while (imgsensor_info.i2c_addr_table[i] != 0xff) {
        ...
        do {
			...
			#endif
			printk("OV12a10B i2c write id: 0x%x, sensor id: 0x%x\n", imgsensor.i2c_write_id,sensor_id);
            if (sensor_id == imgsensor_info.sensor_id) {
                LOG_INF("OV12a10B i2c write id: 0x%x, sensor id: 0x%x\n", imgsensor.i2c_write_id,sensor_id);
                break;
            }
            LOG_INF("OV12a10B Read sensor id fail: 0x%x, id: 0x%x\n", imgsensor.i2c_write_id,sensor_id);
            retry--;
        } while(retry > 0);
        i++;
        if (sensor_id == imgsensor_info.sensor_id)
...
{
	LOG_INF("@@fanfan  OV12A10B_MIPI_RAW_SensorInit() \n");
